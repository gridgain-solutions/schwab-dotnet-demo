...
SCHWAB_DEMO_DOCKERFILE=${SCHWAB_DEMO_HOME}/build/Dockerfile.java-server
...
docker build -f ${SCHWAB_DEMO_DOCKERFILE} -t sdemo-java-server:latest .
# docker build -f ${SCHWAB_DEMO_DOCKERFILE} --progress plain --no-cache -t sdemo-java-server:latest .

docker tag sdemo-java-server:latest dgneubert/sdemo-java-server:latest
docker push dgneubert/sdemo-java-server:latest

# Run the docker image to test it.
echo -e "\nRun local docker image: sdemo-java-server\n"
docker run -it -e CONFIG_URI=/app/config/server-config.xml --rm --network host sdemo-java-server