...
SCHWAB_DEMO_DOCKERFILE=${SCHWAB_DEMO_HOME}/build/Dockerfile.java-base
...
# Build the docker image: schwab-java-base:latest
docker build -f ${SCHWAB_DEMO_DOCKERFILE} -t sdemo-java-base:latest .
# docker build -f ${SCHWAB_DEMO_DOCKERFILE} --progress plain --no-cache -t sdemo-java-base:latest .

docker tag sdemo-java-base:latest dgneubert/sdemo-java-base:latest
docker push dgneubert/sdemo-java-base:latest

# Run the docker image to test it.
echo -e "\nRun local docker image: sdemo-java-base\n"
docker run -it --rm --network host sdemo-java-base
# docker run -it -e CONFIG_URI=/app/config/server-config.xml --rm --network host sdemo-java-base